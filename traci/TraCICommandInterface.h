...
namespace TraCIConstants {
    ...
    constexpr uint8_t RESPONSE_VAR_POSITION = 0x42;
    constexpr uint8_t CMD_SET_VEHICLE_VARIABLE = 0xC4;
    constexpr uint8_t CMD_CHANGELANE = 0x13;
}
namespace veins {
...
public:
    ...
    Coord getVehiclePosition(const std::string& vehicleId);
    ...
    int getNumLanes(const std::string& edgeId) const;
    int getVehicleCountOnLane(const std::string& laneId) const;
    std::list<std::string> getVehicleIds();
    void setZoom(const std::string& viewId, double zoomLevel);
    ...
    public:
        ...
        double getDistanceToTLS(const std::string& tlsId);
    ...
    public:
        explicit Trafficlight(TraCICommandInterface* traci, const std::string& trafficLightId);
//        void setPhase(const std::string& trafficLightId, int phase);
//        int getGreenPhase(const std::string& trafficLightId);
...
//        std::list<std::list<std::tuple<std::string, std::string>>> getControlledLinks() const;
        ...
        std::vector<TraCITrafficLightProgram::Logic> getCompleteRedYellowGreenDefinition() const;
        void setPhase(int phaseIndex);
        void setProgram(const std::string& programId);
    private:
        std::unique_ptr<Trafficlight> trafficlightInterface;
    };
    Trafficlight trafficlight(std::string trafficLightId)
    {
        return Trafficlight(this, trafficLightId);
    }
    Trafficlight* getTrafficlightCommandInterface(const std::string& trafficLightId);
