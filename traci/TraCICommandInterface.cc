...
void TraCICommandInterface::setZoom(const std::string& viewId, double zoomLevel) {
    uint8_t commandId = 0x88;
    uint8_t variableId = 0x12;

    TraCIBuffer buf = connection.query(
            commandId, TraCIBuffer() << viewId << variableId << zoomLevel);
}

int TraCICommandInterface::getNumLanes(const std::string& edgeId) const{
    TraCIBuffer buf = connection.query(CMD_GET_EDGE_VARIABLE, TraCIBuffer() << VAR_LANE_NUMBER << edgeId);
    int numLanes;
    buf >> numLanes;
    return numLanes;
}
int TraCICommandInterface::getVehicleCountOnLane(const std::string& laneId) const{
    TraCIBuffer buf = connection.query(CMD_GET_EDGE_VARIABLE, TraCIBuffer() << LAST_STEP_VEHICLE_NUMBER << laneId);
    int vehicleCount;
    buf >> vehicleCount;
    return vehicleCount;
}
...
std::list<std::string> TraCICommandInterface::getVehicleIds() {
    // TraCI 명령 실행
    TraCIBuffer buf = connection.query(
        TraCIConstants::CMD_GET_SIM_VARIABLE, // 명령 코드
        TraCIBuffer() << TraCIConstants::ID_LIST // 변수 ID
    );
    ...
    for (int i = 0; i < count; ++i) {
        std::string id;
        buf >> id;
        vehicleIds.push_back(id); // 차량 ID 추가
    }
    return vehicleIds;
}

void TraCICommandInterface::Vehicle::changeLane(int laneIndex, double duration) {
    // TraCI 명령 생성
    TraCIBuffer buf = connection->query(
        TraCIConstants::CMD_SET_VEHICLE_VARIABLE, // 명령 ID
        TraCIBuffer() << TraCIConstants::VAR_LANE_CHANGE                // 차량 ID
                      << nodeId   // 변수 ID
                      << laneIndex              // 목표 차선 인덱스
                      << duration               // 지속 시간
    );
//    EV << "Vehicle "<< nodeId << " changed to lane " << laneIndex
//            << " for duration " << duration << " seconds." << endl;
}
...
double TraCICommandInterface::Vehicle::getDistanceToTLS(const std::string& tlsId) {
    auto nextTls = getNextTls();
    for (const auto& tlsInfo : nextTls) {
        if (std::get<0>(tlsInfo) == tlsId) {
            return std::get<2>(tlsInfo); // 거리 반환
        }
    }
    return -1.0; // TLS ID를 찾지 못한 경우
}
...
//std::list<std::list<std::tuple<std::string, std::string>>> TraCICommandInterface::Trafficlight::getControlledLinks() const {
//    TraCIBuffer buf = connection->query(CMD_GET_TL_VARIABLE, TraCIBuffer() << TL_CONTROLLED_LINKS << trafficLightId);
//    std::list<std::list<std::tuple<std::string, std::string>>> result;
//    buf >> result;
//    return result;
//}
...
TraCICommandInterface::Trafficlight* TraCICommandInterface::getTrafficlightCommandInterface(const std::string& trafficLightId){
    return new Trafficlight(this, trafficLightId);
}

TraCICommandInterface::Trafficlight::Trafficlight(TraCICommandInterface* traci, const std::string& trafficLightId)
    : traci(traci), trafficLightId(trafficLightId){
    connection = &traci->connection;
    }

void TraCICommandInterface::Trafficlight::setPhase(int phaseIndex){;
    TraCIBuffer buf = connection->query(CMD_SET_TL_VARIABLE, TraCIBuffer() << TL_PHASE_INDEX << trafficLightId << static_cast<int32_t>(phaseIndex));
    ASSERT(buf.eof());
}

void TraCICommandInterface::Trafficlight::setProgram(const std::string& programId){;
    TraCIBuffer buf = connection->query(CMD_SET_TL_VARIABLE, TraCIBuffer() << TL_PROGRAM << trafficLightId << programId);
    ASSERT(buf.eof());
}

std::vector<TraCITrafficLightProgram::Logic> TraCICommandInterface::Trafficlight::getCompleteRedYellowGreenDefinition() const{
    TraCIBuffer buf = connection->query(CMD_GET_TL_VARIABLE, TraCIBuffer() << TYPE_TL_LOGIC << trafficLightId);
    uint8_t variable;
    buf >> variable;
    ASSERT(variable == TYPE_TL_LOGIC);
    std::vector<TraCITrafficLightProgram::Logic> logics;
    buf >> logics;
    return logics;
}
...
Coord TraCICommandInterface::Vehicle::getPosition(){
    uint8_t variableId = VAR_POSITION;
    TraCIBuffer buf = traci->connection.query(CMD_GET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId);

    uint8_t cmdLength;
    buf >> cmdLength;
    uint8_t commandIdResp;
    buf >> commandIdResp;
    ASSERT(commandIdResp == RESPONSE_GET_VEHICLE_VARIABLE);

    uint8_t variableIdResp;
    buf >> variableIdResp;
    ASSERT(variableIdResp == variableId);

    std::string objectIdResp;
    buf >> objectIdResp;
    ASSERT(objectIdResp == nodeId);

    uint8_t responseTypeId;
    buf >> responseTypeId;
    ASSERT(responseTypeId == POSITION_2D);

    double x, y;
    buf >> x >> y;

    return traci->connection.traci2omnet(TraCICoord(x, y));
}
